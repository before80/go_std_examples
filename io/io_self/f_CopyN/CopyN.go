...
func main() {
	fmt.Println("情况1：Copy 中的 src 实现了`WriteTo`接口 且 dst 实现了`ReaderFrom`接口 ")
	...
	fmt.Println("情况2：Copy 中的 src 实现了`WriteTo`接口 但 dst 没有实现`ReaderFrom`接口 ")
	...
	fmt.Println("情况3：Copy 中的 src 没有实现`WriteTo`接口 但 dst 实现了`ReaderFrom`接口 ")
	...
	fmt.Println("情况4：Copy 中的 src 没有实现`WriteTo`接口 且 dst 没有实现`ReaderFrom`接口 ")
...
// Output:
//情况1：Copy 中的 src 实现了`WriteTo`接口 且 dst 实现了`ReaderFrom`接口
//起始文件字节数： 0
//调用了MyWriter1的ReadFrom方法
//发生错误： r's is neither  *MyReader1 nor *MyReader2, current r's type is *io.LimitedReader
//Copy操作后文件字节数： 0
//情况2：Copy 中的 src 实现了`WriteTo`接口 但 dst 没有实现`ReaderFrom`接口
//起始文件字节数： 0
//Copy操作后文件字节数： 5
//情况3：Copy 中的 src 没有实现`WriteTo`接口 但 dst 实现了`ReaderFrom`接口
//起始文件字节数： 0
//调用了MyWriter1的ReadFrom方法
//发生错误： r's is neither  *MyReader1 nor *MyReader2, current r's type is *io.LimitedReader
//Copy操作后文件字节数： 0
//情况4：Copy 中的 src 没有实现`WriteTo`接口 且 dst 没有实现`ReaderFrom`接口
//起始文件字节数： 0
//Copy操作后文件字节数： 5
